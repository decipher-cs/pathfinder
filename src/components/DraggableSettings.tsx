...
export const DraggableSettings = memo(() => {
  ...
  return (
    <div
      className={"absolute z-50 flex max-h-[80svh] w-[40ch] flex-col overflow-hidden rounded-b-md"}
      style={{
    ...
    >
      <SettingHandle
        ...
        toggleBodyCollapsed={handleCollapse}
        setPosition={setPosition}
      />

      {/* Collapsing Body */}
      <SettingBody isBodyCollapsed={isCollapsed} />
    </div>
  )
})
...
/* Handle and title */
const SettingHandle = memo((props: HandleProps) => {
  /*
  ...
  return (
    <div className="flex justify-between rounded-t-sm bg-neutral-800 px-3 py-2 text-neutral-300">
      ...
      <button onClick={props.toggleBodyCollapsed} className="cursor-pointer">
        <span className="sr-only">collapse or open settings</span>
        {props.isBodyCollapsed ? <ChevronsUpDownIcon /> : <ChevronsDownUpIcon />}
      </button>
    </div>
  )
})
SettingHandle.displayName = "SettingHandle"

/* Body */
const SettingBody = memo((props: { isBodyCollapsed: boolean }) => {
  const { isBodyCollapsed } = props
  ...
  return (
    <div
      className={twMerge(
        clsx(isBodyCollapsed ? "h-0 py-0" : "h-max py-3"),
        "grid max-h-full gap-6 overflow-y-scroll bg-neutral-300 px-3 accent-neutral-700"
      )}
...
SettingBody.displayName = "SettingBody"
